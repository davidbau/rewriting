...
class GanRewriteApp(labwidget.Widget):
    ...
    def __init__(self, gw, mask_dir=None, size=256, num_canvases=9):
        super().__init__(style=dict(border="0", padding="70px 50px",
                                    display="inline-block", width="1280px",
    ...
    def widget_html(self):
        ...
        <div>
        <style>
        #notebook {{ padding: none; }}
        #notebook-container {{ padding: none; width: 100%; }}
        div.input {{ display: none; }}
        div.prompt {{ min-width: 0; width: 0; padding: 0; margin: 0; overflow: hidden; }}
        div.ctx_tray div {{ white-space: nowrap; }}
        div.cell.selected:before {{ width: 0; }}
        .edit_mode div.cell.selected:before {{ width: 0; }}
        div.end_space {{ min-height: 0; }}
        ...
        <div style="
          width:{(self.size + 2) * 2 + 20}px;
          display:inline-block;
          vertical-align:top"
          class="rwa-tray"><!-- context tray -->
        <div style="display:block;
          width:{(self.size + 2) * 2}px;
          vertical-align:top;
          "><!-- bottom of left -->
        <div style="display:block;
          padding-top:20px;
        ...
        <div style="width:{(self.size + 2) * 4 + 150}px;">
        <hr style="border:1px solid gray; background-color: white">
        </div>

        <div style="width:{(self.size + 2) * 4 + 150}px; text-align: center;
